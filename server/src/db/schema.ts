...
export const responseCache = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_cache_unique").on(table.chatbotId, table.queryHash),
...
export const dailyQuests = pgTable(
  "daily_quests",
  {
    ...
    description: text("description"),
    questType: varchar("quest_type", { length: 50 }).notNull(), // "message_count", "correct_answer", "streak"
  ...
  (table) => [
    index("idx_quests_chatbot_date").on(table.chatbotId, table.date),
  ]
);
...
export const questProgress = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_quest_student_unique").on(table.questId, table.studentId),
  ]
);

// ─── Parent Reports (Phase 6) ─────────────────────────
...
export const parentReports = pgTable(
  "parent_reports",
  {
    ...
    childId: text("child_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    reportType: varchar("report_type", { length: 20 }).notNull(), // "weekly", "monthly"
...
export const notificationPreferences = pgTable(
  "notification_preferences",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    channel: varchar("channel", { length: 20 }).notNull(), // "email"
    frequency: varchar("frequency", { length: 20 }).notNull().default("weekly"),
    isEnabled: boolean("is_enabled").notNull().default(true),
    metadata: jsonb("metadata"), // { email: "...", ... }
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  },
  (table) => [
    uniqueIndex("idx_notif_user_channel").on(table.userId, table.channel),
  ]
);
