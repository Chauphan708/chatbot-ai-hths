...
app.all("/api/auth/*", (req, res) => {
  // Skip toNodeHandler for OPTIONS as we already handled it above
...
app.use("/api/teacher", teacherQuestsRouter);
app.use("/api/teacher", analyticsRouter);
app.use("/api/parent", parentRouter);
app.use("/api/parent", parentReportsRouter);
...
// ─── Start Server ───────────────────────────────
app.listen(env.PORT, () => {
  // Khởi chạy bộ lập lịch báo cáo tự động cho phụ huynh
  try {
    initReportScheduler();
  } catch (err) {
