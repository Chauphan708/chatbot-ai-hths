...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sessions = await db
      ...
      .from(chatSessions)
      .innerJoin(chatbots, eq(chatSessions.chatbotId, chatbots.id))
      .leftJoin(classes, eq(chatbots.classId, classes.id))
      .where(eq(chatSessions.studentId, getParam(req, "id")))
      .orderBy(desc(chatSessions.startedAt))
      .limit(20);

    // Lọc: Nếu bot thuộc về một lớp, PH phải là thành viên ĐÃ DUYỆT của lớp đó
    const filteredSessions = [];
    for (const s of sessions) {
      if (s.chatbot.classId) {
        const [membership] = await db
          .select()
          .from(classMembers)
          .where(and(
            eq(classMembers.classId, s.chatbot.classId),
            eq(classMembers.userId, req.user!.id),
            eq(classMembers.isVerified, true)
          ));
        if (membership) filteredSessions.push(s.session);
      } else {
        // Bot cá nhân (không thuộc lớp) - PH xem được nếu đã link cha-con
        filteredSessions.push(s.session);
      }
    }
