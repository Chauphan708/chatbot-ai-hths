...
/** GET /teacher/api-key — Kiểm tra trạng thái API key */
router.get(
  "/api-key",
  asyncHandler(async (req, res) => {
    ...
    const key = teacher?.geminiApiKey;
    res.json({
      success: true,
      data: {
        hasKey: !!key,
...
/** PUT /teacher/api-key — Lưu API key Gemini */
router.put(
  "/api-key",
  asyncHandler(async (req, res) => {
    const { apiKey } = req.body;
    if (!apiKey || typeof apiKey !== "string" || apiKey.length < 10) {
      throw new AppError(400, "API key không hợp lệ");
    }
...
/** DELETE /teacher/api-key — Xóa API key */
router.delete(
  "/api-key",
  asyncHandler(async (req, res) => {
    await db
