...
export function StudentChatPage() {
  ...
  const [studentProgress, setStudentProgress] = useState<{ totalXp: number; level: number; streakDays: number } | null>(null);
  const [showGamification, setShowGamification] = useState(false);

  const loadProgress = useCallback(async () => {
    if (!shareCode) return;
    try {
      const res = await chatApi.getProgress(shareCode);
      setStudentProgress(res.data || null);
    } catch {
      // Silently ignore
    }
  }, [shareCode]);

  // Load bot info & progress
  useEffect(() => {
    if (!shareCode) return;
    const load = async () => {
      try {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      rec.onerror = (event: any) => {
        ...
        showToast("Không nhận dạng được giọng nói. Thử lại nhé!", "error");
  ...
  const handleSendImage = async ({
    ...
    if (!shareCode || remaining <= 0) return;

    const userMessage: ChatMessage = {
      id: `temp-${Date.now()}`,
      sessionId: sessionId || "",
      role: "student",
      content: message ? `[📷 Ảnh bài tập] ${message}` : "[📷 Ảnh bài tập]",
      isVoice: false,
      flagged: false,
      createdAt: new Date().toISOString(),
    };
    ...
    try {
      const res = await chatApi.sendImage(shareCode, {
        image,
        mimeType,
        message,
        sessionId: sessionId || undefined,
      ...
      showToast(
        err instanceof Error ? err.message : "Lỗi gửi ảnh bài tập",
        "error"
      );
    } finally {
      setIsTyping(false);
  ...
  return (
    <div className="chat-layout">
      ...
      <div className="chat-header">
        ...
        <div
          className="chat-header__stats"
          onClick={() => setShowGamification(true)}
          style={{ cursor: "pointer" }}
          title="Xem bảng xếp hạng & nhiệm vụ"
        >
          ...
          <div className="xp-bar-container">
            ...
            <span>L{studentProgress?.level ?? 1}</span>
                  ...
                  width: `${Math.min(
                    ...
                    Math.max(
                      ...
                      (((studentProgress?.totalXp ?? 0) -
                        ((studentProgress?.level ?? 1) - 1) *
                          ((studentProgress?.level ?? 1) - 2) *
                          50) /
