...
import { Plus, Users, CheckCircle, Clock, UserPlus, Edit2, Trash2, Filter } from "lucide-react";
...
export function ClassManagementPage() {
  ...
  // Member Filter State
  const [memberFilter, setMemberFilter] = useState<'all' | 'student' | 'parent'>('all');
    ...
    <DashboardLayout role="teacher">
        ...
        <div className="w-full lg:w-2/3">
          {selectedClass ? (
            <div className="flex flex-col gap-6">
              ...
              {/* Members Table */}
              <GlassCard padding="sm" className="overflow-hidden flex flex-col gap-4">
                <div className="flex items-center justify-between p-2">
                  <div className="flex gap-2">
                    <button 
                      className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${memberFilter === 'all' ? 'bg-primary text-white' : 'bg-white/5 text-secondary hover:bg-white/10'}`}
                      onClick={() => setMemberFilter('all')}
                    >
                      Tất cả
                    </button>
                    <button 
                      className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${memberFilter === 'student' ? 'bg-primary text-white' : 'bg-white/5 text-secondary hover:bg-white/10'}`}
                      onClick={() => setMemberFilter('student')}
                    >
                      Học sinh
                    </button>
                    <button 
                      className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${memberFilter === 'parent' ? 'bg-primary text-white' : 'bg-white/5 text-secondary hover:bg-white/10'}`}
                      onClick={() => setMemberFilter('parent')}
                    >
                      Phụ huynh
                    </button>
                  </div>
                  <div className="text-xs text-secondary font-medium">
                    <Filter size={14} className="inline mr-1" /> {filteredMembers.length} thành viên
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-glass-white/5 border-b border-glass-border">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-sm font-semibold text-right">Hành động</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {loadingMembers ? (
                        ...
                        <tr>
                          <td colSpan={5} className="py-12 text-center text-secondary text-sm">Lớp chưa có thành viên phù hợp</td>
                        </tr>
                        ...
                        <tr key={m.id} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-4 py-4">
                            {m.isVerified ? (
                              ...
                              <div className="flex items-center gap-1 text-warning text-xs font-medium animate-pulse">
      ...
      {showCreateUser && (
        <div className="modal-overlay">
          <GlassCard padding="lg" className="w-full max-w-md animate-in fade-in zoom-in duration-200">
